...
public class FintechServiceImpl extends FintechServiceGrpc.FintechServiceImplBase {
  ...
  public Future<TransferMoneyHolder> checkBalance(TransferMoneyHolder holder) {
    ...
    //    fintechDA
    //        .selectUserForUpdate(userId)
    //        .setHandler(
    //            senderAsyncResult -> {
    //              if (senderAsyncResult.succeeded()) {
    //                User userAuth = senderAsyncResult.result();
    //                holder.setUserAuth(userAuth);
    //                long balance = userAuth.getBalance();
    //                if (balance >= holder.getRequest().getAmount()) {
    //                  future.complete(holder);
    //                  fintechDA
    //                      .selectUserForUpdate(holder.getRequest().getReceiver())
    //                      .setHandler(
    //                          receiverAsyncResult -> {
    //                            if (receiverAsyncResult.succeeded()) {
    //
    // holder.setReceiverBalance(receiverAsyncResult.result().getBalance());
    //                            } else {
    //                              future.fail(receiverAsyncResult.cause());
    //                            }
    //                          });
    //                } else {
    //                  future.fail(
    ...
    //                }
    //              } else {
    //                future.fail(senderAsyncResult.cause());
    //                log.error(
    //                    "get user auth failed, cause={}",
    //                    ExceptionUtil.getDetail(userAsyncResult.cause()));
    //              }
